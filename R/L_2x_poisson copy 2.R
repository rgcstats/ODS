...
#' @param logistic If TRUE, y is assumed to follow a bernoulli distribution with logit link conditional on x, otherwise y are normal
...
L.2x.poisson <- function(beta,sd,prob.x2,mu0.x1,mu1.x1,sd.x1,
                           ys,x1s,x2s,pi.fn,log=FALSE,pi.s,Rquad=100,
                         verbose=FALSE,logistic=FALSE){
  n <- length(ys)
  l.misspecified <- log(prob.x2)*sum(x2s) + log(1-prob.x2)*sum(1-x2s) +
    sum( dnorm( x=x1s , mean=mu0.x1*(1-x2s)+mu1.x1*x2s , sd=sd.x1 , log=TRUE ) )
  if(!logistic){
    l.misspecified <- l.misspecified + sum( dnorm(x=ys,mean=as.vector(beta[1]+beta[2]*x1s+beta[3]*x2s),sd=sd,log=TRUE) )
  }
  if(logistic){
    mu <- make.link("logit")$linkinv(as.vector(beta[1]+beta[2]*x1s+beta[3]*x2s))
    l.misspecified <- l.misspecified + sum(log(mu[ys==1])) + sum(log(1-mu[ys==0])) - 100*n*(sd-1)^2
    # the sd^2 term is added so that sd just gets estimated as 0 in the logit model
  }
