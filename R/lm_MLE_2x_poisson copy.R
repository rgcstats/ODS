...
#' @export
lm.MLE.2x.poisson <- function(ys,x1s,x2s,pi.s,pi.fn,Rquad=100,start.beta,start.sd){
  # get starting values
  ...
  modfit <- optim(par=startpar,
                  fn=function(par){
                    L.2x.poisson(beta=par[1:3],sd=exp(par[4]),prob.x2=exp(par[5])/(1+exp(par[5])),
                         mu0.x1=par[6] , mu1.x1=par[7] ,sd.x1=exp(par[8]),
                         ys=ys,x1s=x1s,x2s=x2s,pi.fn=pi.fn,
                         log=TRUE,Rquad=Rquad)
                  },
