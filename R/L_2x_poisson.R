...
#' @param Rquad number of quadrature points (not used)
...
#'        pi.fn=eg.pi.fn,R=10,log=TRUE)
#' @export
L.2x.poisson <- function(beta,sd,prob.x2,mu0.x1,mu1.x1,sd.x1,
                           ys,x1s,x2s,pi.fn,log=FALSE,Rquad=100,
                         verbose=FALSE){
  n <- length(ys)
  ...
  # easy because y|x2=0 and y|x2=1 are both normal
  ...
  integrand <- function(y,mean,sd){
    pi.fn(y) * dnorm(y,mean=mean,sd=sd)
  }
  pi.bar0 <- integrate(integrand,-Inf,Inf,mean=mu0.y,sd=sd.condx2.y)$value # E[pi|x2=0]
  pi.bar1 <- integrate(integrand,-Inf,Inf,mean=mu1.y,sd=sd.condx2.y)$value # E[pi|x2=1]
