#' Calculates maximum empirical likelihood estimators for linear regression
#'
...
#' if(!log) return(out)})
#' @export
lm.el <- function(ys,xs,pi.fn,N,R=100,tol=1){
  # get starting values
  pi.s <- pi.fn(ys)
  ...
  modfit <- optim(par=c(lm.piwt$coef,log(sd.start)),
                  fn=function(par,ys,pi.fn,N,R){
                    L.el(ys=ys,mean=as.vector(xs%*%par[1:p]),sd=exp(par[p+1]),
                       pi.fn=pi.fn,N=N,R=R,log=TRUE,tol=tol)
                  },
                  ys=ys,pi.fn=pi.fn,N=N,R=R,control=list(fnscale=-1) )
