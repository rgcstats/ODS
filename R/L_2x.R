...
#' @export
L.2x <- function(beta,sd,prob.x2,mu0.x1,mu1.x1,sd.x1,
                 ys,x1s,x2s,N,p.s,pi.fn=NULL,specs=NULL,log=FALSE,pi.s,R=100,
                 all.errors.y,all.unif.x2,all.subsamp.unif,all.errors.x1,
                 verbose=FALSE){
  argg <- c(as.list(environment()))
  if(is.null(pi.fn)) return(do.call(L.2x.inefficient,argg)) else
    return(do.call(L.2x.efficient,argg))
}
